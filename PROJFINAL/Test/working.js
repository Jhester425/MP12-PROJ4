...
async function fetchGeoapifyPlaces(poblacion) {
    ...
    try {
        ...
        if (data.length > 0) {
            ...
            // Example: Searching for places of interest near the selected town
            const categories = "tourism,leisure,sport";
            const geoapifyUrl = `https://api.geoapify.com/v2/places?categories=${categories}&filter=circle:${townLon},${townLat},10000&apiKey=${geoapifyApiKey}`;
            ...
            placesContainer.innerHTML = "<h3>Lugares de Interés</h3>"; // Reset content

            if (placesData.features.length === 0) {
                placesContainer.innerHTML += "<p>No se encontraron lugares de interés cercanos.</p>";
            } else {
                placesData.features.forEach(item => {
                    const place = item.properties;
                    const placeElement = document.createElement("div");
                    placeElement.classList.add("place-item");
                    placeElement.innerHTML = `
                        <h4>${place.name || "Sin nombre"}</h4>
                        <p><strong>Categoría:</strong> ${place.categories.join(", ")}</p>
                        <p><strong>Dirección:</strong> ${place.address_line1 || "Dirección no disponible"}</p>
                    `;
